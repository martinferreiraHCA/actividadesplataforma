...
async function libreIniciar() {
  ...
  worker.onmessage = (ev) => {
    ...
    if (m.tipo === 'estado') {
      L.ocupado = false;
      L.integrados = m.integrados; L.perdidos = m.perdidos; L.cuadros = m.cuadros; L.segmentos = m.segmentos; L.reencontrados = m.reencontrados || 0;
      dibujarLibre(m);
      const c = m.calidad;
...
const PRESETS = {
  ultra: { modo: 'libre', libLado: 300, libVoxel: 3, libEsc: 4, libBilateral: true, libDistancia: 0, libRefinar: 1, optSuavizado: 2, optSuavizadoTipo: 'bilateral', optRelleno: 'solido', optReducir: 'q200000', optAgujeros: true,
    nota: 'Ultra fino: seguimiento ágil (160×120, vóxeles de 3 mm, para poder moverse rápido) y, al terminar, refinado final a 1 mm: las vistas guardadas a resolución completa se reajustan contra el modelo y se vuelven a fundir en un volumen fino sólo alrededor de la pieza. Filtro adaptativo del sensor, fusión ponderada por ángulo y distancia, suavizado bilateral que conserva los rasgos y malla optimizada por QEM a 200 mil triángulos. Kinect a 50–60 cm, movimientos muy lentos (2–4 cuadros por segundo). Pensado para caras, manos, piezas con detalle fino.' },
  cara: { modo: 'libre', libLado: 300, libVoxel: 3, libEsc: 4, libBilateral: true, libDistancia: 0, libRefinar: 1.5, optSuavizado: 1, optSuavizadoTipo: 'bilateral', optRelleno: 'solido', optReducir: 0, optAgujeros: true,
    nota: 'Cara y gestos, máximo detalle: seguimiento ágil de 3 mm y refinado final a 1,5 mm con las vistas guardadas a resolución completa, más filtro de ruido. El Kinect mide más fino cuanto más cerca: trabajá a 55–65 cm (no menos de 50). La persona sostiene la expresión sin moverse; recorré despacio de oreja a oreja pasando por arriba y por debajo del mentón. Al terminar tarda un minuto en refinar.' },
