...
export function generarPromptFichas({ tema, nivel, cantidad, plataforma, enfoque, notas, catalogo, infantil }) {
  ...
  const enfoqueTexto = {
    guia: 'GUÍA PASO A PASO (tutorial): los ítems NO son ejercicios sueltos sino PASOS ordenados para construir UN MISMO proyecto completo (en Scratch un juego; en micro:bit un proyecto con la placa, ej. un contador, un dado, un juego con los botones; en código de texto un programa que crece paso a paso). Cada paso agrega una parte, con "teoria:" explicando el concepto del paso y "codigo:" con el código NUEVO de ese paso. En Scratch, el código de CADA paso empieza con "personaje: Nombre" para que quede claro A QUIÉN se le ponen los bloques, y la consigna dice DÓNDE va lo nuevo ("agregale al Gato estos bloques, debajo de los que ya tiene", "creá este código nuevo en el Perro"...) y qué debería pasar al probarlo. El último paso deja el proyecto terminado y funcionando. NO dediques pasos a "agregar personajes" ni a "elegir el fondo": la guía genera sola una portada de preparación con los personajes, el fondo y una imagen del escenario. Para que esa imagen salga bien, en los primeros pasos definí la posición inicial de cada personaje con "ir a x: () y: ()" y su tamaño con "fijar tamaño al () %".',
