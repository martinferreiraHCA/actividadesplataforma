...
const PREFIJO_PEER = 'hca-sombras-';
...
function clienteId() {
  let id = null;
  try { id = localStorage.getItem('sombras_cliente'); } catch (_) {}
  if (!id) { id = 'e' + Math.random().toString(36).slice(2, 10) + Date.now().toString(36); try { localStorage.setItem('sombras_cliente', id); } catch (_) {} }
  return id;
}
...
function abrirDB() {
  ...
  dbPromesa = new Promise((resolve) => {
    ...
    let req;
    try { req = indexedDB.open('sombras-recortadas', 1); } catch (_) { resolve(null); return; }
    req.onupgradeneeded = () => {
      const db = req.result;
...
export function nuevaAula(nombre) {
  ...
  return { codigo: generarCodigo(), nombre: nombre || 'Mi aula', creada: ahora, actualizada: ahora, estudiantes: {} };
}
...
export function hospedarAula(aula, alEvento) {
  ...
  function iniciar() {
    if (!tienePeer()) { estado = 'sin-peer'; emitir('estado', { estado, detalle: 'No se cargó PeerJS: el aula sólo funciona con archivos .json.' }); return; }
    cerrado = false;
    ...
    peer.on('connection', conn => {
      ...
      conn.on('data', async msg => {
        ...
        if (msg.t === 'hola') {
          ...
          if (!clave) { conn.send({ t: 'rechazo', motivo: 'Falta el nombre.' }); return; }
          const previa = conexiones.get(clave);
          if (previa && previa !== conn && previa.open) { try { previa.send({ t: 'rechazo', motivo: 'Entraste desde otra computadora: esta sesión se cerró.' }); previa.close(); } catch (_) {} }
    ...
    peer.on('error', err => {
      ...
      if (tipo === 'unavailable-id') { estado = 'codigo-ocupado'; emitir('estado', { estado, detalle: 'Ese código ya está en uso en otra computadora (¿el aula quedó abierta en otra pestaña?).' }); return; }
  ...
  return {
    ...
    async quitarEstudiante(clave) {
      const c = conexiones.get(clave);
      if (c) { try { c.send({ t: 'rechazo', motivo: 'El docente te quitó del aula.' }); c.close(); } catch (_) {} conexiones.delete(clave); }
...
function describirError(err) {
  ...
  if (t === 'browser-incompatible') return 'Este navegador no soporta conexiones directas (WebRTC).';
  if (t === 'network' || t === 'server-error' || t === 'socket-error' || t === 'socket-closed') return 'No se pudo llegar al servidor de señales (¿la red bloquea la conexión?). Se reintenta solo; mientras tanto se puede trabajar con archivos .json.';
  if (t === 'peer-unavailable') return 'No hay ningún aula abierta con ese código. Pedile al docente que la abra (tiene que tener la página abierta).';
  return (err && err.message) || 'Error de conexión.';
}
...
export function entrarAula(codigo, nombre, alEvento) {
  ...
  function conectar() {
    if (cerrado) return;
    if (!tienePeer()) { estado = 'sin-peer'; emitir('estado', { estado, detalle: 'No se cargó PeerJS: guardá el .json y mandáselo al docente.' }); return; }
    estado = 'conectando'; emitir('estado', { estado });
